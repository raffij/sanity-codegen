...
function createClient({
  ...
  async function query(query) {
    ...
    const response = await jsonFetch(`https://${projectId}.${useCdn ? 'apicdn' : 'api'}.sanity.io/v1/data/query/${dataset}?${searchParams.toString()}`, { // conditionally add the authorization header if the token is present
      ...(preview && {
        headers: {
